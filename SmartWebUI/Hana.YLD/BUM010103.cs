...
namespace Hana.YLD
{
    public partial class BUM010103 : Miracom.SmartWeb.UI.Controls.udcCUSReport002
    {
        private bool b_load_flag;
        //DACrux.YMS.Admin.frmCompositeFrequencyMapView oro = null;
        
        //private string sCustDevice = null;
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //else if (MPCF.Trim(txtCRRID.Text) != string.Empty)
                //{
                //    // CARR ID 전송
                //    in_node.ProcStep = '3';
                //    in_node.AddString("CRR_ID", txtCRRID.Text);
                //}

                ViewLotInfo(procStep);
                ...
                //spdWafBin.ActiveSheet.OperationMode = FarPoint.Win.Spread.OperationMode.Normal;
                //CmnFunction.FitColumnHeader(spdWafBin);

            }
            catch (Exception ex)
            {
                CmnFunction.ShowMsgBox("BUM010103.btnView_Click() : " + ex.ToString());
            }
            finally
            {
        ...
        private void LotInfo(DataTable lot_info)
        {
            spdSpread.Sheets[0].Cells[0, 1].Value = lot_info.Rows[0]["MAT_ID"];
            spdSpread.Sheets[0].Cells[1, 1].Value = lot_info.Rows[0]["FLOW"];
            spdSpread.Sheets[0].Cells[2, 1].Value = lot_info.Rows[0]["OPER"];
            spdSpread.Sheets[0].Cells[3, 1].Value = lot_info.Rows[0]["QTY_1"];
            spdSpread.Sheets[0].Cells[4, 1].Value = "0";
            spdSpread.Sheets[0].Cells[5, 1].Value = lot_info.Rows[0]["LOT_TYPE"];
            spdSpread.Sheets[0].Cells[6, 1].Value = lot_info.Rows[0]["LOT_STATUS"];
            spdSpread.Sheets[0].Cells[7, 1].Value = lot_info.Rows[0]["OPER_IN_TIME"];
            spdSpread.Sheets[0].Cells[8, 1].Value = " ";
            spdSpread.Sheets[0].Cells[9, 1].Value = " ";
            spdSpread.Sheets[0].Cells[10, 1].Value = " ";
            spdSpread.Sheets[0].Cells[11, 1].Value = lot_info.Rows[0]["LAST_COMMENT"];
            spdSpread.Sheets[0].Cells[12, 1].Value = lot_info.Rows[0]["HOLD_CODE"];
            spdSpread.Sheets[0].Cells[13, 1].Value = " ";

            spdSpread.Sheets[0].Cells[1, 3].Value = lot_info.Rows[0]["LOT_CMF_2"];
            spdSpread.Sheets[0].Cells[2, 3].Value = " ";
            spdSpread.Sheets[0].Cells[3, 3].Value = " ";
            spdSpread.Sheets[0].Cells[4, 3].Value = " ";
            spdSpread.Sheets[0].Cells[5, 3].Value = " ";
            spdSpread.Sheets[0].Cells[6, 3].Value = lot_info.Rows[0]["LOT_PRIORITY"];

            spdSpread.Sheets[0].Cells[9, 3].Value = lot_info.Rows[0]["END_RES_ID"];
            
            //txtCRRID.Text = MPCF.Trim(out_node.GetString("CRR_ID"));            
        }

        private void ViewLotInfo(char ProcStep)
        {
            DataTable dt = null;
            ...
            try
            {
                ...
                sQuery.AppendLine("    *  ");
                sQuery.AppendLine("FROM MWIPLOTSTS@RPTTOMES ");                
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    sOprDefQuery.Replace(":FACTORY", "'" + dt.Rows[0]["FACTORY"] + "'");
                    sOprDefQuery.Replace(":OPER", "'" + dt.Rows[0]["OPER"] + "'");
                    ...
                    if (dt.Rows[0]["LOT_CMF_1"].ToString().Trim() == "")
                    {
                        s_mother_lot = txtLotID.Text.Trim();
                    }
                    else
                    {
                        s_mother_lot = dt.Rows[0]["LOT_CMF_1"].ToString(); 
                    }
        ...
        private void ViewWaferResultYMS()
        {
            ...
            try
            {
                ...
                //sLotStsQuery.AppendLine("FROM MWIPLOTSTS@TESTMES ");    //RWIPLOTSTS?                
        ...
        private bool ViewMapServerInfo(string table, string key1)
        {
            ...
            {                
                ...
                //sQuery.Replace(":FACTORY", "'HMKB1'");  //TEST
                sQuery.Replace(":FACTORY", "'" + cdvFactory.Text + "'");
        ...
        //        sQuery.Replace(":LOT_ID", "'" + txtLotID.Text + "'");
        //        sQuery.Replace(":OPER", "'" + cdvOper.SelectedValueToQueryString + "'");
        ...
        //private bool Get_Cust_Device_YMS()
        //{
        //    DataTable dt = null;
        //    StringBuilder sQuery = new StringBuilder();
        //    int i = 0;
        ...
        //            sQuery.AppendLine("    SELECT");
        //            sQuery.AppendLine("        MAX(WAFHIS.CUST_DEVICE)");
        //            sQuery.AppendLine("    FROM TQC_WAFER@MES2YMS WAFHIS");
        //            sQuery.AppendLine("    WHERE 1=1");
        //            sQuery.AppendLine("        AND WAFHIS.WAFER_ID IN (SELECT WAFER_ID FROM CWIPLOTWAF WHERE LOT_ID=:LOT_ID)");
        //            sQuery.AppendLine("        AND WAFHIS.TESTAREA=:OPER");
        //            sQuery.AppendLine("        AND WAFHIS.RETEST_IDX_REV=0");
        //        }
        //        else
        //        {
        ...
        //            sQuery.AppendLine("    SELECT");
        //            sQuery.AppendLine("        MAX(WAFHIS.CUST_DEVICE)");
        //            sQuery.AppendLine("    FROM TQC_M_WAFER@MES2YMS WAFHIS");
        //            sQuery.AppendLine("    WHERE 1=1");
        //            sQuery.AppendLine("        AND WAFHIS.WAFER_ID IN (SELECT WAFER_ID FROM CWIPLOTWAF WHERE LOT_ID=:LOT_ID)");
        //            sQuery.AppendLine("        AND WAFHIS.RETEST_IDX_REV=0");
        //        }
        ...
        //        if (dt.Rows.Count > 0)
        //        {
        //            sCustDevice = dt.Rows[0][0].ToString();
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        private void ClearData(Miracom.SmartWeb.UI.Controls.udcFarPoint spread, char ProcStep)
        {
            ...
            {
                CmnFunction.ShowMsgBox("YLD041003() : " + ex.ToString());
            }
        }
        ...
        #region [ Form Event ]
        private void YLD041003_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                // MAP 서버 연결 정보
                tpucBinMapView1.MAP_SERVER_IP = map_server_ip;      //"12.230.55.151"
                tpucBinMapView1.MAP_SERVER_PORT = map_server_port;  //7513

                InitSpdColumnHeader();
                InitValueByRegister();
                //txtFactory.Text = GlobalVariable.gsFactory;

                //oro = new DACrux.YMS.Admin.frmCompositeFrequencyMapView();
        ...
        private void spdWafBin_CellClick(object sender, FarPoint.Win.Spread.CellClickEventArgs e)
        {
            ...
            try
            {
                ...
                //if (cdvOperation.DescText == "IQC" || spdSpread.Sheets[0].Cells[2, 1].Value.ToString() == "B1000")
                //    tpucBinMapView1.DrawWaferMap(spdWafBin.ActiveSheet.Cells[e.Row, (int)COL_WAFER.WAFER_ID].Text,
                //                                 "IQC");
                //else
                //    tpucBinMapView1.DrawWaferMap(spdWafBin.ActiveSheet.Cells[e.Row, (int)COL_WAFER.WAFER_ID].Text,
                //                                 "MULTIPROBE");
        ...
        //        //oro.Draw("PTA01", sCustDevice, wafer_data, draw_flag);
        //    }
        //    catch (Exception ex)
        //    {
        //        //throw ex;
