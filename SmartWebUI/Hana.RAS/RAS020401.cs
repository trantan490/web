...
namespace Hana.RAS
{
    /// <summary>
    /// 클  래  스: RAS020401<br/>
    /// 클래스요약: Trend Yield<br/>
    /// 작  성  자: 미라콤 양형석<br/>
    /// 최초작성일: 2008-12-17<br/>
    /// 상세  설명: Trend Yield<br/>
    /// 변경  내용: <br/>
    /// </summary>
    public partial class RAS020401 : Miracom.SmartWeb.UI.Controls.udcCUSReport002
    {
        public RAS020401()
        {
            InitializeComponent();
        ...
        private void GridColumnInit()
        {
            ...
            spdData.RPT_AddBasicColumn("Classification", 0, 14, Visibles.True, Frozen.False, Align.Center, Merge.True, Formatter.String, 80);

            spdData.RPT_AddDynamicColumn(udcFromToDate, 0, 15, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Number, 80);
        ...
        private string MakeSqlString()
        {
            ...
            {
                strYield += "     , DECODE(SUM(OUT_" + i.ToString() + "), 0, 0, TRUNC(SUM(OUT_" + i.ToString() + ")/SUM(IN_" + i.ToString() + ")*100,3)) YIELD_" + i.ToString() + "\n";
                strIn += "     , SUM(IN_" + i.ToString() + ") IN_" + i.ToString() + "\n";
                strOut += "     , SUM(OUT_" + i.ToString() + ") OUT_" + i.ToString() + "\n";
                strLoss += "     , SUM(LOSS_" + i.ToString() + ") LOSS_" + i.ToString() + "\n";
            }   

            string strDecode = string.Empty;
            strDecode += udcFromToDate.getDecodeQuery("DECODE(CUTOFF_DT", "IN_QTY, 0)", "IN_").Replace(", DECODE(", "              , DECODE(");
            strDecode += udcFromToDate.getDecodeQuery("DECODE(CUTOFF_DT", "OUT_QTY, 0)", "OUT_").Replace(", DECODE(", "              , DECODE(");
            strDecode += udcFromToDate.getDecodeQuery("DECODE(CUTOFF_DT", "LOSS, 0)", "LOSS_").Replace(", DECODE(", "              , DECODE(");
            ...
            {
                case "DAY":
                    strSqlString.Append("                 SELECT SUBSTR(TRAN_TIME, 1, 8) CUTOFF_DT " + "\n");
                    break;
                case "WEEK":
                    strSqlString.Append("                 SELECT GET_WORK_DATE(TRAN_TIME, 'W') CUTOFF_DT " + "\n");
                    break;
                case "MONTH":
                    strSqlString.Append("                 SELECT SUBSTR(TRAN_TIME, 1, 6) CUTOFF_DT " + "\n");
                    break;
                default :
                    strSqlString.Append("                 SELECT SUBSTR(TRAN_TIME, 1, 8) CUTOFF_DT " + "\n");
                    break;
            ...
            strSqlString.Append("                      , RES.IN_QTY IN_QTY " + "\n");
            strSqlString.Append("                      , RES.OUT_QTY OUT_QTY " + "\n");
            strSqlString.Append("                      , RES.LOSS LOSS " + "\n");
            strSqlString.Append("                   FROM ( " + "\n");
            strSqlString.Append("                             SELECT PREV.TRAN_TIME, PREV.FACTORY, PREV.RES_ID, PREV.MAT_ID, PREV.LOT_ID, RES.RES_GRP_1, RES.RES_GRP_2, RES.RES_GRP_3, RES.RES_GRP_5, RES.RES_GRP_6, RES.RES_GRP_7 " + "\n");
            strSqlString.Append("                                  , PREV.RES_HIST_SEQ PREV_SEQ, POST.RES_HIST_SEQ POST_SEQ, PREV.OPER PREV_OPER, POST.OPER POST_OPER, IN_QTY, OUT_QTY, (IN_QTY-OUT_QTY) LOSS " + "\n");
            strSqlString.Append("                                  , (TO_DATE(OUT_TIME, 'YYYYMMDDHH24MISS') - TO_DATE(IN_TIME, 'YYYYMMDDHH24MISS'))*24 WORK_TIME " + "\n");
            ...
            strSqlString.Append("                              WHERE 1=1 " + "\n");
            strSqlString.Append("                                AND RES.FACTORY = PREV.FACTORY " + "\n");
            strSqlString.Append("                                AND RES.RES_ID = PREV.RES_ID " + "\n");
            ...
            strSqlString.Append("                    AND RES.IN_QTY <> 0 AND RES.OUT_QTY <> 0 " + "\n");

            //#region " 조회 조건 : Product"

            //if (cdvProduct.Text != "ALL" && cdvProduct.Text != "")
            //    strSqlString.AppendFormat("                 AND MAT.MAT_ID {0}" + "\n", cdvProduct.SelectedValueToQueryString);
        ...
        private void ShowChart(int rowCount)
        {
            ...
            //YIELD
            double yield = udcChartFX1.RPT_4_AddData(spdData, new int[] { 0 }, yield_columns, SeriseType.Rows);

            //LOSS
            double loss = udcChartFX1.RPT_4_AddData(spdData, new int[] { 3 }, yield_columns, SeriseType.Rows);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //by John
                //1.Griid 합계 표시
                int[] rowType = spdData.RPT_DataBindingWithSubTotalAndDivideRows(dt, 0, 0, 15, 15, 4, udcFromToDate.SubtractBetweenFromToDate+1, btnSort);
                ...
                //3. Total부분 셀머지
                spdData.RPT_FillDataSelectiveCells("Total", 0, 14, 0, 4, true, Align.Center, VerticalAlign.Center);
        ...
        {
            int nStartValueColumn = 15;     // 값들이 시작되는 열 위치
