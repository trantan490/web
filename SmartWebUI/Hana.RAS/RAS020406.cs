...
namespace Hana.RAS
{
    ...
    /// </summary>
    /// 
    public partial class RAS020406 : Miracom.SmartWeb.UI.Controls.udcCUSReport002
    {
        DataTable global_res_id = null;
        ...
        private Boolean CheckField()
        {
            ...
            {
                CmnFunction.ShowMsgBox(RptMessages.GetMessage("STD026", GlobalVariable.gcLanguage));
                return false;
            }

            //if (udcRASCondition3.Text.TrimEnd() == "ALL")
            //{
            //    CmnFunction.ShowMsgBox(RptMessages.GetMessage("STD027", GlobalVariable.gcLanguage));
            //    return false;
            //}
        ...
        private void GridColumnInit()
        {
            ...
            spdData.RPT_AddBasicColumn("파트", 0, 1, Visibles.False, Frozen.False, Align.Center, Merge.True, Formatter.String, 80);
            ...
            {
                spdData.RPT_AddBasicColumn("Operation", 0, 14, Visibles.True, Frozen.False, Align.Center, Merge.True, Formatter.String, 70);
            }


            if (cdvFactory.txtValue != "")
            {
                global_res_id = CmnFunction.oComm.GetFuncDataTable("DYNAMIC", MakeSqlString1());

                for (int i = 0; i < global_res_id.Rows.Count; i++)
                {
                    spdData.RPT_AddBasicColumn(global_res_id.Rows[i][0].ToString(), 0, (15 + i), Visibles.True, Frozen.False, Align.Center, Merge.False, Formatter.String, 80);
                }
            }
        ...
        {
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Team in charge", "RES_GRP_1", "A.RES_GRP_1", "NVL((SELECT DATA_1 FROM MGCMTBLDAT WHERE FACTORY = A.FACTORY AND TABLE_NAME = 'H_DEPARTMENT' AND ROWNUM=1 AND KEY_1 = RES_GRP_1), '-') AS TEAM", true);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Part", "RES_GRP_2", "A.RES_GRP_2", "NVL((SELECT DATA_1 FROM MGCMTBLDAT WHERE FACTORY = A.FACTORY AND TABLE_NAME = 'H_DEPARTMENT' AND ROWNUM=1 AND KEY_1 = RES_GRP_2), '-') AS PART", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Operation", "RES_GRP_3", "A.RES_GRP_3", "RES_GRP_3", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Maker", "RES_GRP_5", "A.RES_GRP_5", "RES_GRP_5", false);            
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Model", "RES_GRP_6", "A.RES_GRP_6", "RES_GRP_6", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Equipment", "RES_ID", "A.RES_ID", "RES_ID", false);            

            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Customer", "MAT_GRP_1", "A.MAT_GRP_1", "MAT_GRP_1", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Family", "MAT_GRP_2", "A.MAT_GRP_2", "MAT_GRP_2", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Package", "MAT_GRP_3", "A.MAT_GRP_3", "MAT_GRP_3", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Type1", "MAT_GRP_4", "A.MAT_GRP_4", "MAT_GRP_4", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Type2", "MAT_GRP_5", "A.MAT_GRP_5", "MAT_GRP_5", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("LD Count", "MAT_GRP_6", "A.MAT_GRP_6", "MAT_GRP_6", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Density", "MAT_GRP_7", "A.MAT_GRP_7", "MAT_GRP_7", false);
            ((udcTableForm)(this.btnSort.BindingForm)).AddRow("Generation", "MAT_GRP_8", "A.MAT_GRP_8", "MAT_GRP_8", false);
        }
        ...
        private string MakeSqlString()
        {
            ...
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("            , MAX(MODEL" + i + ") AS MODEL" + "\n");                                
            }
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("            , TRUNC(DECODE( SUM(IN_QTY" + i + "), 0, 0, ((SUM(IN_QTY" + i + ") - NVL(SUM(LOSS_QTY" + i + "),0))/SUM(IN_QTY" + i + "))*100),3) YIELD " + "\n");
            }
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("            , NVL(SUM(LOSS_QTY" + i + "),0) AS  LOSS " + "\n");
            }
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("            , SUM(IN_QTY" + i + ") AS I " + "\n");
            }
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("            , SUM(OUT_QTY" + i + ")AS O " + "\n");
            }
            ...
            for (int i = 0; i < global_res_id.Rows.Count; i++)
            {
                strSqlString.Append("                    , DECODE(A.RES_ID, '" + global_res_id.Rows[i][0].ToString() + "', A.RES_GRP_6, ' ') AS MODEL" + i + "\n");
                strSqlString.Append("                    , DECODE(A.RES_ID, '" + global_res_id.Rows[i][0].ToString() + "', B.LOSS_QTY, 0 ) AS LOSS_QTY" + i + "\n");
                strSqlString.Append("                    , DECODE(A.RES_ID, '" + global_res_id.Rows[i][0].ToString() + "', A.OPER_IN_QTY_1, 0) AS IN_QTY" + i + "\n");
                strSqlString.Append("                    , DECODE(A.RES_ID, '" + global_res_id.Rows[i][0].ToString() + "', A.END_QTY_1, 0) AS OUT_QTY" + i + "\n");
            }

            strSqlString.Append("                 FROM (SELECT A.FACTORY, A.MAT_ID, A.LOT_ID     " + "\n");
            ...
            strSqlString.Append("                            , A.END_QTY_1, A.OPER_IN_QTY_1, A.END_RES_ID AS RES_ID " + "\n");
            ...
            strSqlString.Append("                          AND LOSS_CODE " + cdvLoss.SelectedValueToQueryString + "\n");
            strSqlString.Append("                      ) B                " + "\n");
            strSqlString.Append("                WHERE 1 = 1          " + "\n");
            strSqlString.Append("                  AND A.LOT_ID = B.LOT_ID(+)    " + "\n");
            strSqlString.Append("                  AND A.RES_ID = B.RES_ID(+)    " + "\n");
            strSqlString.Append("                  AND A.RES_ID IS NOT NULL  " + "\n");
            strSqlString.AppendFormat("             GROUP BY {0}  " + "\n",QueryCond2);
            strSqlString.Append( "                    , A.RES_ID, A.RES_GRP_6, LOSS_QTY, OPER_IN_QTY_1, END_QTY_1, A.FACTORY " + "\n");
            strSqlString.Append("              )  A" + "\n");            
            strSqlString.AppendFormat("             GROUP BY {0}" + "\n", QueryCond1);
            strSqlString.Append( "                    , A.FACTORY " + "\n");
        ...
        private string MakeSqlString1()             
        {
            ...
            strSqlString.Append("               AND RES_TYPE NOT IN ('DUMMY')" + "\n");
            // ---- RES 상세 조회 ---- //             
            if (udcRASCondition1.Text != "ALL" && udcRASCondition1.Text != "")
        ...
        private void ShowChart(int rowCount)
        {
            ...
            udcChartFX1.RPT_3_OpenData(2, global_res_id.Rows.Count);
            int[] yield_columns = new Int32[global_res_id.Rows.Count];
            int[] loss_columns = new Int32[global_res_id.Rows.Count];
            int[] columnsHeader = new Int32[global_res_id.Rows.Count];
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //int[] rowType = spdData.RPT_DataBindingWithSubTotalAndDivideRows(dt, 0, 0, 0, 17, 4, global_res_id.Rows.Count, btnSort);

                spdData.DataSource = dt;
                spdData.RPT_DivideRows(15, 5, global_res_id.Rows.Count);
                ...
                spdData.RPT_FillColumnData(14, new string[] { "Model", "Yield", "Loss", "Input", "Output" });
