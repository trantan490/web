...
namespace Hana.PRD
{
    public partial class PRD010702 : Miracom.SmartWeb.UI.Controls.udcCUSReport002
    {
        ...
        /// <summary>
        /// 클  래  스: PRD010702<br/>
        /// 클래스요약: 외주(도금) 진도 관리<br/>
        /// 작  성  자: 하나마이크론 임종우<br/>
        /// 최초작성일: 2010-04-20<br/>
        /// 상세  설명: 외주 도금 진도관리 조회<br/>
        /// 변경  내용: <br/>   
        /// 2012-08-21-임종우 : 도금 업체 정보 없어도 오류 발생 하지 않도록 수정
        /// 2013-10-14-김민우 : LOT TYPE ALL, P%, E% 구분으로 변경
        /// 2018-01-23-임종우 : SubTotal, GrandTotal 백분율 구하기 Function 변경
        /// </summary>
        public PRD010702()
        {
            InitializeComponent();            
        ...
        private void GridColumnInit()
        {
            ...
            if (ckbKpcs.Checked == false) // Kpcs 구분
            {
                ...
                spdData.RPT_AddBasicColumn("Vendor WIP", 0, 8, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Number, 70);
                spdData.RPT_AddBasicColumn("3day Performance", 0, 9, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.String, 80);
                spdData.RPT_AddBasicColumn(dayArray2[0], 1, 9, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Number, 70);
                spdData.RPT_AddBasicColumn(dayArray2[1], 1, 10, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Number, 70);
                spdData.RPT_AddBasicColumn(dayArray2[2], 1, 11, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Number, 70);
            }
            else
            {
                spdData.RPT_AddBasicColumn("plan", 0, 5, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
                spdData.RPT_AddBasicColumn("actual", 0, 6, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
                spdData.RPT_AddBasicColumn("Progress rate", 0, 7, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 50);
                spdData.RPT_AddBasicColumn("Vendor WIP", 0, 8, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
                spdData.RPT_AddBasicColumn("3day Performance", 0, 9, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.String, 80);
                spdData.RPT_AddBasicColumn(dayArray2[0], 1, 9, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
                spdData.RPT_AddBasicColumn(dayArray2[1], 1, 10, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
                spdData.RPT_AddBasicColumn(dayArray2[2], 1, 11, Visibles.True, Frozen.False, Align.Right, Merge.False, Formatter.Double1, 70);
            }

            spdData.RPT_MerageHeaderColumnSpan(0, 9, 3);
            spdData.RPT_AddBasicColumn("TAT", 0, 12, Visibles.True, Frozen.False, Align.Center, Merge.False, Formatter.Double2, 60);
            spdData.RPT_AddBasicColumn("Ratio", 0, 13, Visibles.True, Frozen.False, Align.Center, Merge.False, Formatter.Double2, 60);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                spdData.RPT_SetPerSubTotalAndGrandTotal(1, 6, 5, 7);

                //6. TAT 평균값 구하기(GrandTotal부분 & SubTotal)
                spdData.RPT_SetAvgSubTotalAndGrandTotal(1, 12, 0, false);                
        ...
        private string MakeSqlString()
        {
            ...
            if (ckbKpcs.Checked == false) // Kpcs 구분
            {
                ...
                strSqlString.Append("     , DECODE(SUM(PLAN_QTY), 0, 0, ROUND((SUM(MON_RECV_QTY)/SUM(PLAN_QTY))*100,1)) AS JINDO " + "\n");
                strSqlString.Append("     , SUM(CUST_QTY) AS CUST_QTY " + "\n");
            ...
            {
                strSqlString.Append("     , ROUND(SUM(PLAN_QTY)/1000, 1) AS PLAN_QTY " + "\n");
                strSqlString.Append("     , ROUND(SUM(MON_RECV_QTY)/1000, 1) AS MON_RECV_QTY " + "\n");
                strSqlString.Append("     , DECODE(SUM(PLAN_QTY), 0, 0, ROUND((SUM(MON_RECV_QTY)/SUM(PLAN_QTY))*100,1)) AS JINDO " + "\n");
                strSqlString.Append("     , ROUND(SUM(CUST_QTY)/1000, 1) AS CUST_QTY " + "\n");
                strSqlString.Append("     , ROUND(SUM(DAY_RECV_QTY_1)/1000, 1) AS DAY_RECV_QTY_1 " + "\n");
                strSqlString.Append("     , ROUND(SUM(DAY_RECV_QTY_2)/1000, 1) AS DAY_RECV_QTY_2 " + "\n");
                strSqlString.Append("     , ROUND(SUM(DAY_RECV_QTY_3)/1000, 1) AS DAY_RECV_QTY_3 " + "\n");
            }
                        
            strSqlString.Append("     , SUM(TAT) AS TAT " + "\n");
            strSqlString.Append("     , ROUND(RATIO_TO_REPORT(SUM(MON_RECV_QTY)) OVER () * 100, 2) AS PER " + "\n");            
            ...
            strSqlString.Append("             , SUM(DAY_RECV_QTY_2) AS DAY_RECV_QTY_2 " + "\n");
            strSqlString.Append("             , SUM(DAY_RECV_QTY_3) AS DAY_RECV_QTY_3   " + "\n");
            strSqlString.Append("             , ROUND(DECODE(SUM(DECODE(TAT, 0, 0, 1)), 0, 0, SUM(TAT) / SUM(DECODE(TAT, 0, 0, 1))), 2) TAT " + "\n");            
            ...
            strSqlString.Append("                      , CASE WHEN (HLD.RELEASE_TRAN_TIME > '" + endDate + "' OR TRIM(RELEASE_TRAN_TIME) IS NULL) " + "\n");
            strSqlString.Append("                             THEN HLD.QTY_1 " + "\n");
            strSqlString.Append("                             ELSE 0 " + "\n");
            strSqlString.Append("                         END AS CUST_QTY        " + "\n");

            //strSqlString.Append("                      , DECODE(TRIM(HLD.RELEASE_TRAN_TIME), NULL, HLD.QTY_1, 0) AS CUST_QTY   " + "\n");
            ...
            */
            ...
            if (udcWIPCondition6.Text != "ALL" && udcWIPCondition6.Text != "")
                strSqlString.AppendFormat("   AND LEAD_COUNT {0} " + "\n", udcWIPCondition6.SelectedValueToQueryString);
