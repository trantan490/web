...
#region " Function Definition "
    ...
    public object Clone()
    {
        DBC_MFMBUDRLOC MFMBUDRLOC = null;
        try
        {
            MFMBUDRLOC = new DBC_MFMBUDRLOC(ref _dbc);

            MFMBUDRLOC.FACTORY = this.FACTORY;
            MFMBUDRLOC.GROUP_ID = this.GROUP_ID;
            MFMBUDRLOC.RES_ID = this.RES_ID;
            MFMBUDRLOC.RES_TYPE = this.RES_TYPE;
            MFMBUDRLOC.SEQ = this.SEQ;
            MFMBUDRLOC.LOC_X = this.LOC_X;
            MFMBUDRLOC.LOC_Y = this.LOC_Y;
            MFMBUDRLOC.LOC_WIDTH = this.LOC_WIDTH;
            MFMBUDRLOC.LOC_HEIGHT = this.LOC_HEIGHT;
            MFMBUDRLOC.TEXT = this.TEXT;
            MFMBUDRLOC.TEXT_SIZE = this.TEXT_SIZE;
            MFMBUDRLOC.TEXT_COLOR = this.TEXT_COLOR;
            MFMBUDRLOC.TEXT_STYLE = this.TEXT_STYLE;
            MFMBUDRLOC.TAG_TYPE = this.TAG_TYPE;
            MFMBUDRLOC.BACK_COLOR = this.BACK_COLOR;
            MFMBUDRLOC.CREATE_TIME = this.CREATE_TIME;
            MFMBUDRLOC.UPDATE_TIME = this.UPDATE_TIME;
            MFMBUDRLOC.NO_MOUSE_EVENT = this.NO_MOUSE_EVENT;
            MFMBUDRLOC.SIGNAL_FLAG = this.SIGNAL_FLAG;
        }
        catch(Exception ex)
        {
        ...
        try
        {
            ...
            switch(_step)
            {
                case 1:
                    ...
                    strQuery = "SELECT * FROM MFMBUDRLOC"
                        + " WHERE FACTORY=?"
                        + " AND GROUP_ID=?"
                        + " AND RES_ID=?"
                        + " AND RES_TYPE=?";
        ...
        try
        {
            ...
            switch(_step)
            {
                case 1:
                    ...
                    strQuery = "SELECT COUNT(*) FROM MFMBUDRLOC"
                        + " WHERE FACTORY=?"
                        + " AND GROUP_ID=?"
                        + " AND RES_ID=?"
                        + " AND RES_TYPE=?";
        ...
        try
        {
            ...
            switch(_step)
            {
                case 1:
                    ...
                    strQuery = "DELETE FROM MFMBUDRLOC"
                        + " WHERE FACTORY=?"
                        + " AND GROUP_ID=?"
                        + " AND RES_ID=?"
                        + " AND RES_TYPE=?";
    ...
    public bool InsertData()
    {
        ...
        try
        {
            ...
            strQuery = "INSERT INTO MFMBUDRLOC ("
                + " FACTORY, GROUP_ID, RES_ID, RES_TYPE, SEQ,"
                + " LOC_X, LOC_Y, LOC_WIDTH, LOC_HEIGHT, TEXT,"
                + " TEXT_SIZE, TEXT_COLOR, TEXT_STYLE, TAG_TYPE, BACK_COLOR,"
                + " CREATE_TIME, UPDATE_TIME, NO_MOUSE_EVENT, SIGNAL_FLAG)"
                + " VALUES ("
                + " ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
                + " ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try
        {
            ...
            switch(_step)
            {
                case 1:
                    ...
                    strQuery = "UPDATE MFMBUDRLOC SET"
                        + " SEQ=?, LOC_X=?, LOC_Y=?, LOC_WIDTH=?, LOC_HEIGHT=?,"
                        + " TEXT=?, TEXT_SIZE=?, TEXT_COLOR=?, TEXT_STYLE=?, TAG_TYPE=?,"
                        + " BACK_COLOR=?, CREATE_TIME=?, UPDATE_TIME=?, NO_MOUSE_EVENT=?, SIGNAL_FLAG=?"
                        + " WHERE FACTORY=?"
                        + " AND GROUP_ID=?"
                        + " AND RES_ID=?"
                        + " AND RES_TYPE=?";
